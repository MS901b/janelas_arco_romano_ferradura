function validaExpressao(valor)
{
	...
	valor = String(valor).replace(/[0-9\+\-\*\/\(\)π\.]/gi,"");
	valor = String(valor).replace(/pi/gi,"");
	valor = String(valor).replace(/√/gi,"");
	valor = String(valor).replace(/\^/gi,"");
	valor = String(valor).replace(/[²³]/gi,"");
	valor = String(valor).replace(/\s/gi,"");
...
function contaParenteses(valor)
{
	...
	var novovalor1 = String(valor).replace(/\)/g,"");
	var novovalor2 = String(valor).replace(/\(/g,"");
...
function processaExpressao(valor)
{
	...
	if ((valor!='') && (validaExpressao(valor)))
	{
		...
		valor = String(valor).replace(/π/gi,'Math.PI');
		valor = String(valor).replace(/\-\-/gi,'- -');
